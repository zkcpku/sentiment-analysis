...
class SenTree(ParentedTree):
    ...
    @staticmethod
    def getTrees(file= opt.small_path,vocabOutFile=None, vocabIndicesMapFile=None):
...
# trn = SenTree.getTrees(opt.train_path,vocabOutFile = opt.vocab_path) # 第一次解析的时候需要生成词向量
trn = SenTree.getTrees(opt.train_path,vocabIndicesMapFile=opt.vocab_path) # 修改后
dev = SenTree.getTrees(opt.dev_path,vocabIndicesMapFile=opt.vocab_path)

vocab_size = SenTree.vocabSize 
