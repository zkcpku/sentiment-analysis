...
# 修改此处数据地址
with open(opt.train_path,encoding = "utf-8") as f:
	train_csv = pd.read_csv(f,error_bad_lines=False)
train_csv = train_csv.drop(['Unnamed: 0','type','file'],axis=1)
train_csv.columns = ["review","sentiment"]
train_csv = train_csv[train_csv.sentiment != 'unsup']
train_csv['sentiment'] = train_csv['sentiment'].map({'pos': 1, 'neg': 0})
...
def generate_vocab(use_old = True):
	if use_old:
		...
		train_phrases = train_csv['review']
...
def pad_samples(features, maxlen=opt.max_len, PAD=0):
	'''
...
def generate_set(valid_per = 0.2):
	...
	if opt.use_old_dataloader:
		...
		train_set = [clean_string(w) for w in train_csv['review']]
...
if __name__ == '__main__':
	...
	# train_phrases = train_csv['review']
	# phrases_cleaned = [clean_string(s) for s in train_phrases]
	# print(len(phrases_cleaned))
	# max_len = 2514
	# for i in phrases_cleaned:
	# 	if max_len < len(i):
	# 		max_len = len(i)
